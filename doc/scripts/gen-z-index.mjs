...
function parseTiers(src) {
  const arrayMatch = src.match(/export const Z_INDEX_TIERS[^=]*=\s*\[([\s\S]*?)\];/);
  if (!arrayMatch) {
    throw new Error(`Could not locate "export const Z_INDEX_TIERS = [ ... ]" in ${TOKENS_PATH}`);
  }
  ...
  while ((m = objectRe.exec(body)) !== null) {
    ...
    if (!nameMatch || !valueMatch) {
      throw new Error(`Malformed tier object in Z_INDEX_TIERS (missing name/value): ${obj.trim()}`);
    }
...
function buildBlock(tiers) {
  ...
  lines.push(`   * GENERATED:Z_INDEX — do not hand-edit; run pnpm gen:z-index.`);
  lines.push(`   * Source of truth: src/config/z-index-tokens.ts. Tailwind v4 reads the`);
  lines.push(`   * --z-index-<name> theme key and generates a z-<name> utility. */`);
  lines.push(`  @theme {`);
...
function main() {
  ...
  if (check) {
    if (next !== css) {
      console.error('z-index codegen drift detected: src/styles/global.css is out of date.');
  ...
  if (next === css) {
    console.log(`z-index @theme block already up to date (${tiers.length} tiers); no change.`);
    return 0;
  }
  writeFileSync(CSS_PATH, next);
  console.log(`Wrote z-index @theme block to src/styles/global.css (${tiers.length} tiers).`);
  return 0;
}
