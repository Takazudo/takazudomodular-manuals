...
export default {
  ...
  async preBuild(ctx) {
    ...
    if (locales != null) {
      for (const [key, entry] of Object.entries(/** @type {Record<string, unknown>} */ (locales))) {
        if (
          ...
          throw new Error(`[doc-history] invalid locales entry "${key}": expected { dir: string }`);
        }
      }
    }
    await runDocHistoryMetaStep({
      ...
      locales: locales != null ? /** @type {Record<string,{dir:string}>} */ (locales) : undefined,
    });
  },
  ...
  async postBuild(ctx) {
    try {
      await runDocHistoryPostBuild(
        /** @type {import("@takazudo/zudo-doc/integrations/doc-history").DocHistoryOptions} */ (
          /** @type {unknown} */ (ctx.options)
        ),
