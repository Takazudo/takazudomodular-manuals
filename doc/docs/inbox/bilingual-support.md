...
Today every shipped manual includes both `pages-ja.json` and `pages-en.json` (see sub-issue #97), so the disabled state is not exercised in production. The mechanism is preserved for future additions that may ship JA-only first.
