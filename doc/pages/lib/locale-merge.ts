...
export function mergeLocaleDocs<T extends DocsEntry = DocsEntry>(
  ...
  let fallbackDocs = filteredBase.filter((d) => !localeSlugSet.has(d.data.slug ?? d.id));
 ...
 */
export function mergeCategoryMeta(baseDir: string, localeDir: string): Map<string, CategoryMeta> {
