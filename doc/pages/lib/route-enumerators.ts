...
export function enumerateDocsRoutes(locale: string): string[] {
  ...
  const {
    ...
    navDocs,
    categoryMeta,
  } = resolveNavSource(locale as Locale, undefined, {
...
export function enumerateTagsRoutes(locale: string): string[] {
  ...
  const tagsBase = locale === defaultLocale ? '/docs/tags' : `/${locale}/docs/tags`;
 ...
 */
export function enumerateVersionedRoutes(version: VersionConfig, locale: string): string[] {
  const urls: string[] = [];
  ...
  if (locale === defaultLocale) {
    ...
    const {
      ...
      navDocs,
      categoryMeta,
    } = resolveVersionedLocaleSource(version.slug, version.docsDir, locale as Locale, localeDir, {
      applyDefaultLocaleOnlyFilter: true,
      keepUnlisted: true,
    });
    const tree = buildNavTree(navDocs, locale as Locale, categoryMeta);
