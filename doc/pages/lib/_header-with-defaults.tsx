...
import { docsUrl, navHref, stripBase, versionedDocsUrl, withBase } from '@/utils/base';
 ...
 */
export function HeaderWithDefaults(props: HeaderWithDefaultsProps): JSX.Element {
  const {
  ...
  if (settings.versions && settings.versions.length > 0) {
    ...
    for (const v of settings.versions) {
      versionUrls[v.slug] =
        currentSlug != null ? versionedDocsUrl(currentSlug, v.slug, lang) : versionsPageUrl;
    }
  ...
  const languageSwitcher =
    localeLinks != null
      ? ((<LanguageSwitcher links={localeLinks} />) as unknown as VNode)
      : undefined;
