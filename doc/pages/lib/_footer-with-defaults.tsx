 ...
 */
export function FooterWithDefaults({ lang = defaultLocale }: FooterWithDefaultsProps): VNode {
  ...
  const linkColumns: FooterLinkColumn[] = links.map((column) => ({
    title:
      (column.locales as Record<string, { title: string }> | undefined)?.[lang]?.title ??
      column.title,
    items: column.items.map((item) => ({
      label:
        (item.locales as Record<string, { label: string }> | undefined)?.[lang]?.label ??
        item.label,
  ...
  if (taglist?.enabled) {
    ...
    const localeOverrides = (
      taglist.locales as
        | Record<string, { title?: string; groupTitles?: Record<string, string> }>
        | undefined
    )?.[lang];
