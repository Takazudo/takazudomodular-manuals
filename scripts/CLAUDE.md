...
8. **Markdown → HTML** - Runs `pdf:md-to-html` to convert each page's `content` markdown to `contentHtml` in both `pages-ja.json` and `pages-en.json`. Runs AFTER `pdf:clean-en` so EN html reflects the final cleaned content. Uses the same unified pipeline as `components/markdown-renderer.tsx`: `unified` + `remark-parse` + `remark-gfm` + `remark-rehype` + `rehype-slug` + `rehype-highlight` + `rehype-stringify`. Pages with `hasContent === false` get `contentHtml: ""`. See `scripts/pdf-md-to-html.js`.
...
- Fenced code blocks get `hljs` CSS classes from `rehype-highlight` (e.g., `<code class="hljs language-js">`).
- GFM tables, strikethrough, and task lists are supported via `remark-gfm`.
- Sub 4's island injects this HTML via `dangerouslySetInnerHTML` — the client ships zero markdown JS.
- Structural deviations from react-markdown's custom `components/markdown/*`: no Tailwind classNames, `<table>` not wrapped in scroll div, `<h2>`/`<h3>` lack decorative spans, `<a>` lacks `target="_blank"`. Sub 2's prose CSS targets raw HTML elements.
