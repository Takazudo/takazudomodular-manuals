 ...
 * neighbouring utility buttons (see components/header.tsx `utilityButtonStyles`)
 * so the segmented toggle visually blends into the utility bar.
 */
...
/**
 * Segmented JA | EN control for the header utility bar. Reads the active
 * language via `useLanguage()` and delegates persistence (URL query string +
 * localStorage) to the LanguageProvider's `setLang`.
 *
 * Renders nothing when `manualId` is null so the control stays hidden on the
 * global manual index (`/`). When the manual has no English translation
 * (`hasLanguage(manualId, 'en') === false`), the EN button is visibly dimmed,
 * marked `aria-disabled`, and its click becomes a no-op; a tooltip explains
 * why.
 */
export function LanguageToggle({ manualId }: LanguageToggleProps) {
  const { lang, setLang } = useLanguage();

  if (!manualId) return null;

  const enAvailable = hasLanguage(manualId, 'en');
